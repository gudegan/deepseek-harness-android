...
export function spawnWorker(args) {
  return new Promise((resolve) => {
    const child = spawn(process.execPath, [SCRIPT_PATH, ...args], { stdio: ['ignore', 'pipe', 'pipe'], windowsHide: true })
    let stdout = ''
    let stderr = ''
...
function globalFilesetDirtySync(memoryDir, fileset) {
  ...
  const run = (args) => {
    const r = spawnSync('git', args, { cwd: memoryDir, encoding: 'utf8', timeout: 3000, stdio: ['ignore', 'pipe', 'pipe'], env, windowsHide: true })
...
function runGitSync(dir, args) {
  try {
    const r = spawnSync('git', args, { cwd: dir, encoding: 'utf8', timeout: 3000, stdio: ['ignore', 'pipe', 'pipe'], windowsHide: true })
